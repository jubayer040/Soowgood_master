...
class BeneficiaryViewMedicalHistoryScreen extends StatefulWidget {
  ...
  BeneficiaryViewMedicalHistoryScreen({Key? key, required this.appointmentData})
      : super(key: key);

  @override
  _BeneficiaryViewMedicalHistoryScreenState createState() =>
      _BeneficiaryViewMedicalHistoryScreenState();
}

class _BeneficiaryViewMedicalHistoryScreenState
    extends State<BeneficiaryViewMedicalHistoryScreen> {
  BeneficiaryViewMedicalHistoryController getXController =
      Get.put(BeneficiaryViewMedicalHistoryController());
  ...
  Widget getPrescriptionList(int index) {
    ...
    prescriptionDate = getUiDate(
        getData(getXController.prescriptionList[index].prescriptiondate));
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            getRowData('Appointment Id',
                getData(getXController.prescriptionList[index].appointmentNo)),
            getRowData('Date', prescriptionDate),
            getRowData('Diagnosis',
                getData(getXController.prescriptionList[index].diognosis)),
            Row(
              ...
              children: [
                getIconRowData(
                    ...
                    getData(getXController.prescriptionList[index].id),
                    '',
                    '',
                    ''),
                const SizedBox(
                ...
                getIconRowData(
                  ...
                  getData(getXController.prescriptionList[index].id),
                  widget.appointmentData.serviceProviderId!,
                  prescriptionDate,
                  getData(getXController.prescriptionList[index].diognosis),
                ),
              ],
      ...
      children: [
        Text(
          ...
          style: const TextStyle(
              fontSize: 12, color: Colors.black, fontFamily: 'poppins_regular'),
        ),
        Row(
          mainAxisAlignment: MainAxisAlignment.end,
          children: [
  ...
  ///
  ///
  getIconRowData(String type, IconData icon, String prescriptionId,
      String providerId, String prescriptionDate, String diagnosis) {
    return InkWell(
      onTap: () {
        if (type == 'browser') {
          log('Prescription PDF Name $prescriptionId');
          Get.to(() => OpenInBrowserScreen(
              url:
                  'https://api.soowgood.com/prescriptionpdf/$prescriptionId.pdf')); //open pdf url
        } else {
          Get.to(() => BeneficiaryPrescriptionDetailScreen(
              ...
              children: [
                SizedBox(
                  ...
                  child: ClipRRect(
                    ...
                    child: widget.appointmentData.providerImage == null ||
                            widget.appointmentData.providerImage!.isEmpty
                        ? Image(
                            image: noProfileImage,
                          )
                        : Image.network(
                            '${ApiConstant.fileBaseUrl}${ApiConstant.profilePicFolder}${widget.appointmentData.providerImage}',
                            width: 80.0,
                            height: 80.0,
